...
export function UserMenu({ user, onLogout }: UserMenuProps) {
  ...
  if (!user) {
    return (
      <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors font-body">
        <User size={18} />
        <span>Sign In</span>
      </button>
    );
      ...
      >
        ...
        <div className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center border border-emerald-200 dark:border-emerald-800">
            {user.avatarUrl ? (
                ...
                <span className="text-xs font-bold font-heading">{user.name.charAt(0).toUpperCase()}</span>
            )}
        </div>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 py-1 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 sm:hidden">
            <p className="text-sm font-medium text-slate-900 dark:text-slate-100 truncate font-heading">{user.name}</p>
          </div>
          <button
            onClick={() => {
              onLogout?.();
