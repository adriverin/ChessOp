                ...
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-6">
                    ...
                    <nav className="flex items-center gap-1 sm:gap-2 text-sm">
                        {navItems.map((item) => (
                            <Link
                                ...
                                to={item.path}
                                className={clsx(
                                    'relative flex items-center gap-2 px-3 py-2 rounded-full transition-all border border-transparent cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70',
                                    location.pathname === item.path
                                        ? 'bg-indigo-500/15 text-indigo-600 border-indigo-200 shadow-inner after:absolute after:-bottom-1 after:left-3 after:right-3 after:h-0.5 after:rounded-full after:bg-indigo-500 dark:bg-indigo-500/20 dark:text-indigo-100 dark:border-indigo-500/40 dark:after:bg-indigo-300'
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            type="button"
                            onClick={toggleTheme}
                            className="p-2 rounded-full border border-slate-200 text-slate-700 hover:text-slate-900 hover:border-slate-300 hover:bg-slate-100 transition shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 dark:border-slate-700 dark:text-slate-300 dark:hover:text-white dark:hover:border-slate-500 dark:hover:bg-slate-800/60"
                        ...
                        {!loading && isAuthenticated && !isPremium && (
                            <button
                                type="button"
                                onClick={goToPricing}
                                className="hidden sm:inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-indigo-500 to-indigo-400 text-white shadow-lg shadow-indigo-900/30 hover:shadow-indigo-900/40 hover:brightness-110 cursor-pointer transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900"
                            >
                                Upgrade
                            </button>
                        )}
                        {user?.is_authenticated ? (
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    onClick={() => logout()}
                                    className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-full transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-300/70"
                                    title="Log Out"
                                >
