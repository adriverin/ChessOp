...
function OpeningCard({
    ...
    const prerequisitesMet = useMemo(() => {
        ...
        progress.prerequisites.requiredLevel,
        progress.prerequisites.requiredOpenings,
        progressByOpeningId,
        user.level,
    ])
    ...
    return (
        ...
        >
            <button onClick={() => onToggleExpanded?.(isExpanded ? null : opening.id)} className="w-full text-left">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    ...
                                    <span
                                        className={cx(
                                            ...
                                            tone === 'emerald' &&
                                                'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-200 ring-emerald-100/80 dark:ring-emerald-900/40',
                                            tone === 'amber' &&
                                                'bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-200 ring-amber-100/80 dark:ring-amber-900/40',
                                            tone === 'slate' &&
                                                'bg-slate-100 text-slate-700 dark:bg-slate-900/50 dark:text-slate-200 ring-slate-200/80 dark:ring-slate-700/60'
                                        )}
                                    >
                                        {tone === 'emerald' ? (
                                    ...
                                    <span className="inline-flex items-center justify-center rounded-full bg-slate-100 text-slate-700 dark:bg-slate-900/60 dark:text-slate-200 px-2.5 py-1 text-[11px] font-semibold ring-1 ring-inset ring-slate-200/70 dark:ring-slate-700/60">
                                        {isExpanded ? (
                            ...
                            <div className="mt-4 grid grid-cols-3 gap-2">
                                <Metric label="Variations" value={opening.variationCount} />
                                <Metric label="Due" value={dueReviews} />
                ...
                <div className="border-t border-slate-200/70 dark:border-slate-800">
                    <div className="p-4 sm:p-5 grid gap-4 lg:grid-cols-12">
                        <div className="lg:col-span-7 space-y-3">
                            <div className="rounded-xl bg-slate-50/70 dark:bg-slate-950/40 ring-1 ring-inset ring-slate-200/70 dark:ring-slate-800 p-4">
                                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                                    REQUIREMENTS
                                </p>
                                        ...
                                        <span
                                            className={cx(
                                                ...
                                                user.level >= progress.prerequisites.requiredLevel
                                                    ? 'bg-emerald-50 text-emerald-700 ring-emerald-100/80 dark:bg-emerald-950/30 dark:text-emerald-200 dark:ring-emerald-900/40'
                                                    : 'bg-amber-50 text-amber-700 ring-amber-100/80 dark:bg-amber-950/20 dark:text-amber-200 dark:ring-amber-900/40'
                                            )}
                                        >
                                        ...
                                        <div className="flex flex-col gap-1 items-end">
                                            {progress.prerequisites.requiredOpenings.length === 0 ? (
                                                ...
                                                progress.prerequisites.requiredOpenings.map((req) => {
                                                    const name = openingsById.get(req.openingId)?.name ?? req.openingId
                                                    const current = progressByOpeningId.get(req.openingId)?.state ?? 'locked'
                                                    const ok =
                                                        req.requiredState === 'unlocked'
                                                            ? current === 'unlocked' || current === 'mastered'
                                                    ...
                                                    return (
                                                        <span
                                                            key={`${req.openingId}:${req.requiredState}`}
                                                            className={cx(
                                                                'inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 ring-inset',
                                                                ok
                                                                    ? 'bg-emerald-50 text-emerald-700 ring-emerald-100/80 dark:bg-emerald-950/30 dark:text-emerald-200 dark:ring-emerald-900/40'
                                                        ...
                                                        >
                                                            {name} • {stateLabel(req.requiredState)}
                                                        </span>
                                                    )
                                                })
                                            )}
                                    ...
                                    <div className="mt-3 flex items-start gap-3">
                                        <div
                                            className={cx(
                                                ...
                                                progress.premiumLocked
                                                    ? 'bg-amber-50 text-amber-700 ring-amber-100/70 dark:bg-amber-950/20 dark:text-amber-200 dark:ring-amber-900/40'
                                                    : progress.lockReason
                                            ...
                                            <p className="mt-0.5 text-sm text-slate-600 dark:text-slate-300">
                                                {progress.premiumLocked
                                                    ? (progress.lockReason?.message ??
                                                        `Upgrade to unlock ${opening.name}.`)
                                                    : progress.lockReason
                                                        ? progress.lockReason.message
                                        ...
                                        <>
                                            ...
                                            <div className="rounded-xl bg-slate-50 dark:bg-slate-950/40 ring-1 ring-inset ring-slate-200/70 dark:ring-slate-800 p-3">
                                                ...
                                                <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">
                                                    Your goal is {goals.dailyReviewTarget} reviews/day with a stamina cap
                                                    of {goals.staminaCap}. Prioritize {formatSide(goals.preferredSide)}{' '}
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-sm font-semibold text-slate-900 dark:text-white">
                                            Drill a line, then expand
                                        </p>
                                    </div>
                                    <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-200 ring-1 ring-inset ring-emerald-100/80 dark:ring-emerald-900/40">
                                        <Swords className="h-5 w-5" strokeWidth={2} />
                                    </div>
                                </div>
                                <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                                    Master one opening at a time. Use your daily target, then unlock your next branch.
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                        ...
                                        • {user.totalXp.toLocaleString()} XP • Preferred side:{' '}
                                        <span className="font-semibold text-slate-900 dark:text-white">
                                            {formatSide(goals.preferredSide)}
                                        </span>
                                    </p>
                                </div>
